...
var atomCmd = &cobra.Command{
	Use:   "atom <zip_file_path>",
	Short: "It will open the directory in Atom",
	...
	Example: `uzzo atom demo.zip
		  uzzo atom /Downloads/application.zip`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		commandCode := exec.Command("atom", wd)
		err = commandCode.Run()

		// File not found
		if err != nil {
			log.Fatal("Atom Code executable file not found in %PATH%")
		}
...
func init() {
	...
	atomCmd.PersistentFlags().StringVarP(&File, "file", "f", "", "enter the file dir")

	// Cobra supports local flags which will only run when this command
	// is called directly, e.g.:
	// atomCmd.Flags().BoolP("toggle", "t", false, "Help message for toggle")
}
